...
function LoginScreen({ onLogin }: { onLogin: (name: string, provider: string) => void }) {
  const [phone, setPhone] = useState("");
  return (
    <section className="start-sheet login-screen">
      <PaperDoodles />
      <div className="sheet-header">
        <div className="brand-mark-mini">
          <DoodleIcon type="swords" />
        ...
        <div className="header-text">
          <h1>JOIN <span>ARENA</span></h1>
          <p className="screen-subtitle">Save your rating, climb the ladder, connect with rivals.</p>
        </div>
      </div>
        ...
        <div className="phone-login-card">
          ...
          <div className="form-group" style={{ display: 'flex', gap: '10px' }}>
            <input 
              ...
              style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid #334155', background: 'rgba(255,255,255,0.05)', color: '#f8fafc' }}
            />
            <button className="primary-button" style={{ margin: 0, padding: '0 20px', minHeight: '100%' }} onClick={() => phone && onLogin("Player_" + phone.slice(-4), "phone")}>
              SMS Code ➔
            </button>
          </div>
        </div>

        <div className="social-divider" style={{ textAlign: 'center', margin: '24px 0', borderBottom: '1px solid rgba(255,255,255,0.1)', lineHeight: '0.1em' }}>
          <span style={{ background: '#0d101c', padding: '0 15px', color: '#64748b', fontSize: '0.8rem', fontWeight: 800 }}>OR CONNECT VIA SOCIALS</span>
        </div>
        
        <div className="social-login-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <button className="social-btn facebook" onClick={() => onLogin("FB_Warrior", "facebook")}>
            <svg viewBox="0 0 24 24" width="24" height="24"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z" fill="currentColor"/></svg>
            Facebook
          </button>
          <button className="social-btn instagram" onClick={() => onLogin("Insta_Duelist", "instagram")}>
            <svg viewBox="0 0 24 24" width="24" height="24"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" fill="none" stroke="currentColor" strokeWidth="2"/><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z" fill="none" stroke="currentColor" strokeWidth="2"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
            Instagram
          </button>
          <button className="social-btn snapchat" onClick={() => onLogin("Snap_Ninja", "snapchat")}>
            <svg viewBox="0 0 24 24" width="24" height="24"><path d="M12 2C8 2 5 5 5 10c0 3-1 4-2 5l-1 1h12.5a3 3 0 002.5-3l1-1c-1-1-2-2-2-5 0-5-3-8-7-8z" fill="currentColor" stroke="none"/></svg>
            Snapchat
          </button>
          <button className="social-btn email" onClick={() => onLogin("Email_Mage", "email")}>
            <svg viewBox="0 0 24 24" width="24" height="24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" fill="none" stroke="currentColor" strokeWidth="2"/><polyline points="22,6 12,13 2,6" fill="none" stroke="currentColor" strokeWidth="2"/></svg>
            Email
          </button>
        </div>
      </div>
...
function ProfileScreen({ user, rating, onBack, onLogout }: { user: {name: string, provider: string}, rating: number, onBack: () => void, onLogout: () => void }) {
  const rank = getRatingRank(rating);
  return (
    <section className="start-sheet profile-screen">
      <PaperDoodles />
      <div className="sheet-header" style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
         <Avatar team="player" />
         <div className="header-text">
            <h1 style={{ color: '#f8fafc', margin: 0 }}>{user.name}</h1>
            <p className="screen-subtitle" style={{ margin: '4px 0 0 0' }}>Connected via {user.provider}</p>
         </div>
      </div>
      <div className="profile-stats" style={{ marginTop: '24px' }}>
         <div className="rating-preview-card" style={{ borderColor: rank.color, background: 'rgba(255,255,255,0.03)' }}>
           <h3 style={{ margin: '0 0 8px 0', color: '#94a3b8' }}>Global Ladder Rating</h3>
           <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px' }}>
             <b style={{ color: rank.color, fontSize: '4rem', lineHeight: 1 }}>{rating}</b>
             <span style={{ fontSize: '1.5rem', fontWeight: 800, color: '#f8fafc' }}>{rank.title}</span>
           </div>
         </div>
      </div>
      <div className="mp-actions" style={{ marginTop: '24px', display: 'flex', gap: '12px' }}>
...
function LobbyScreen({
  ...
  return (
    <section className="start-sheet lobby-screen">
      <div className="sheet-header">
        <div className="brand-mark-mini">
        ...
        <div className="header-text" style={{ flex: 1 }}>
          ...
          <p className="screen-subtitle">Wave 2: Room Matchmaking. Choose your arena path.</p>
        </div>
        <button className="ghost-button" onClick={onProfile} style={{ padding: '0 16px', height: '44px' }}>
          👤 Profile
        </button>
      </div>
    ...
    <div className={`notebook-app ${isPlayingLayout ? "arena-playing-layout" : ""} ${isShaking ? "shake-screen" : ""}`}>
      ...
      {phase === "login" && (
        <div className="center-screen">
          <LoginScreen 
            onLogin={(name, provider) => {
              const u = { name, provider };
              setUser(u);
